...
Kmeans_max::Kmeans_max(const arma::mat &data, int p, std::map<int, std::set<int>> &ml_map,
...
Kmeans_max::Kmeans_max(const arma::mat &data, int p,
					std::vector<std::pair<int, int>> &global_ml,
					std::vector<std::pair<int, int>> &global_cl, bool verbose){
    this->p = p;
    this->n = data.n_rows;
...
// pick centroids as nearest to the centroid of the dataset
void Kmeans_max::initCentroids(arma::mat &distances) {

	arma::vec chances = arma::ones(n);
	centroids = arma::zeros(p, d);
	...
	// Take best points near to cluster centroid
    std::vector<std::pair<double, int>> dist; // Pair of (distance, index)
	for (int i = 0; i < n; ++i)
		dist.push_back({std::pow(arma::norm(data.row(i) - center, 2),2), i}); // Store distance and index
...
bool Kmeans_max::assignPoints(std::vector<int> &permutation) {
...
bool Kmeans_max::violateConstraint(int point_i, int cluster_j) {
...
bool Kmeans_max::computeCentroids() {
...
bool Kmeans_max::start(int max_iter, int n_permutations, arma::mat &init_centroids) {
...
// multi-start
bool Kmeans_max::start(int max_iter, int n_start, int n_permutations, arma::mat &distances) {
...
// multi-start
bool Kmeans_max::start(int max_iter, int n_start, int n_permutations) {
	arma::mat distances = compute_distances(data);
	return this->start(max_iter, n_start, n_permutations, distances);
}

bool Kmeans_max::findClustering(int n_start, int n_permutations, arma::mat distances) {
...
double Kmeans_max::getLoss() {
    return loss;
}

// get assignments matrix X
arma::sp_mat Kmeans_max::getAssignments() {
    arma::sp_mat assignments_mat(n, p);
    for (int i = 0; i < n; i++) {
...
// get centroids
arma::mat Kmeans_max::getCentroids() {
    return centroids;
}

// mss objective function
double Kmeans_max::objectiveFunction() {
